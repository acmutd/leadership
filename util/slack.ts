...
const fetchUsers = async (slackToken: string): Promise<any> => {
  const config = {
    headers: {
 ...
 */
export const fetchID = async (name: string, slackToken: string = process.env.SLACK_TOKEN): Promise<string> => {
  const users = await fetchUsers(slackToken).then(filterCurrentUsers).then(mapNamesToIds);
